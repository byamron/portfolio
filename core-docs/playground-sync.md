...
> **Status:** Accepted. Supersedes `ui-playground/docs/promote-demo-to-arcade.md`
> (which describes an obsolete file-copy workflow).
>
> Mirror location:
> - portfolio → `core-docs/playground-sync.md` (this file)
> - ui-playground → `docs/cross-repo-sync.md`
>
...
The portfolio either imports these from the submodule directly, or
renders submodule code that does. Renaming, restructuring, or changing
exports breaks the portfolio without a corresponding PR pair:

- **`@playground/palette`** *(direct import)* — named exports `bg`,
  `demoPalettes`, `text`
- **`@playground/demos/<slug>/<Component>`** *(direct import)* — each
  demo's named export matches its file name (e.g. `PageTransition` in
  `PageTransition.tsx`)
- **`@playground/components/DevPanel`** *(transitive via demos)* —
  `DevPanel`, `DevSlider`, `DevToggle`, `DevButtonGroup`, `DevButton`,
  `DevTextInput`, `DevDivider`. Most demos depend on these; a breaking
  change cascades.
- **`@playground/utils/webgl`** *(transitive via slide-unlock)* — WebGL
  helper signatures.
...
editing the playground. Direct vs transitive matters for *finding* the
break (direct = grep portfolio src/, transitive = grep demo source),
not for whether it matters.
...
portfolio's 9-demo allowlist (and renumber catalogue codes, and prefix
paths with `/arcade/`) without modifying submodule source, the gallery
shell must live portfolio-side with its own `demos.ts`.
...
- The playground's gallery shell stays active for workshop use (Museum
  ...
  not in a deprecation relationship. See D7.
...
> flag this per the workshop-side responsibilities in §8 of the
> playground mirror — if it doesn't, that's worth a callback before
> bumping.
...
4. Add to `staticRoutes` in `scripts/generate-sitemap.ts`.
...
`/dvd` is a current example: it's a 301 to `/arcade/dvd-bounce` because
an external Twitter post references it. If DVD Bounce ever gets
