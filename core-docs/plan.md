...
- [ ] Manual click-through of the settings-panel toggle and cursor-mode picker — CDP-based click automation was unreliable in the prototyping sandbox this round, so those paths are verified structurally (button never moves; wired to real contexts) but not via a live interaction recording
