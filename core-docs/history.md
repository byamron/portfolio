...
## 2026-08-19 — V2 home prototype round 4: face-centered crops, intensity-linked glow, back-arrow close, staff-review fixes

**Branch:** `simplified-home-prototype`. Continues the `/new` prototype.

**Summary:** Fixed five specific pieces of Ben's page-feedback on `/new`, then ran `/flow:staff-review` (4 parallel lenses, weighted toward push-further) on the result per his explicit ask — "how can we push this further? I want this to be a fun playground for customization."

**Feedback fixes:**
- **Face-centered square thumbnails.** The 36px home thumbnail was using the tall 3:4 site portraits at `object-fit: cover`, cropping subjects off-center (esp. `portrait` and `pizza`, where the face sits well outside a naive center-crop). Generated real square crops per accent (`public/images/portrait-square-*.jpg`) by visually estimating each photo's face-center fraction and cropping a face-centered 700×700 window, downsized to 240×240.
- **Intensity-linked glow.** Thumbnail's aura now uses the exact formula the sidebar's trigger dot already uses (`SidebarThemeControls.tsx`): `box-shadow: 0 0 lerp(0,14,t)px color-mix(var(--swatch) lerp(0,0.5,t)%, transparent)`. Previously a static warm-gold glow unrelated to accent or intensity.
- **Heading rewrite.** "Ben Yamron is a product designer. Currently designing AI for scientific research at Consensus." → one clause with a comma, no trailing period after the arrow glyph (was reading as "...Consensus↗.").
- **Two new inline links.** "patient experiences" (→ mochi-subscriptions) and "internal tools" (→ mochi-ai-tooling) in the Mochi paragraph, using their real preview videos.
- **Copy fix.** "Worked with" → "I worked with".
- **Close affordance redesigned.** Was an X icon dimming the thumbnail photo on click; Ben asked for a back-arrow instead, positioned above the thumbnail rather than overlaid on it, matching the case-study back button. Implemented, then staff-review caught that the naive version of "above" (a normal-flow row) reintroduced round 3's exact "thumbnail moves" bug — see below.

**Staff-review findings and fixes (4 parallel lenses: staff-engineer, UX, design-engineer, push-further):**
- **BLOCKER (UX, live-measured).** The new back-button row sat in normal document flow above the thumbnail's anchor box; since that box is vertically centered via `marginBlock: auto`, growing its height on open redistributed the auto margins and shifted the whole column — the reviewer measured a 32px jump via a live CDP click test. Fixed: the back button is now `position: absolute` at a negative top offset within the *same* never-resizing anchor box as the thumbnail (rendered after it in DOM order, so it doesn't affect flow), so opening/closing the panel cannot move anything. Re-verified live post-fix: thumbnail top before/after open differs by 0.7px (sub-pixel animation noise, not a shift).
- **BLOCKER (UX, WCAG 2.4.3).** No keyboard path to the back button (it was earlier in DOM order than the thumbnail, so forward-Tab from the thumbnail skipped past it) and no focus restoration on close. Fixed: back button now renders after the thumbnail in DOM order; `thumbRef.focus()` called from both the Escape handler and the back button's `onClick`. Verified live: Tab from the thumbnail lands on "Back to home"; clicking it returns focus to "Customize appearance".
- **BLOCKER (design-engineer).** Every control in the new settings panel had zero hover feedback — `IconButton`'s opacity was keyed to selection state only, swatches had no hover style at all — a direct regression against the sidebar's own documented rule ("hover = spatial feedback, selection = state feedback"). Fixed: added hover states to both, plus the site's one intentional overshoot — the Spring click-press curve (`cubic-bezier(0.34, 1.56, 0.64, 1)`, design-language.md § Imagery) — reused (not reinvented) on every control's press/release. This doubles as the push-further lens's top inline-cheap suggestion for "pop more."
- **NIT.** Thumbnail glow only ported the box-shadow half of the sidebar's two-channel formula; the opacity half was silently dropped (applying it to a photo would visibly fade Ben's face while dragging the slider — reasonable to omit, but the code comment overclaimed fidelity). Fixed: opacity channel redirected to the border's alpha instead, giving the same two-channel read without dimming the image; comment now discloses the deviation explicitly.
- **NIT.** Settings panel had no surface chrome (no background, blur, border, or dividers) — read as "a generic settings form bolted onto an otherwise crafted site." Fixed: lightweight glass surface (accent-tinted fill + `blur(1px)` + border, same frost-recipe ingredients as the rest of the site) and hairline dividers between the three control groups.
- **NIT.** `VideoFill` had no `onError` fallback (unlike `LottieFill`, which falls back to a blank tile) — a 404'd video would leave a permanent transparent hole. Fixed, matching the existing pattern.
- **NIT.** Lottie fetch/parse failures were silently swallowed with no console signal. Added `console.warn`.
- **NIT (staff-engineer, real bug).** The Arcade link (`href: '/playground'`, an internal SPA route) was misclassified as "external" because `isExternal` was derived from "href is present" rather than "href is an http(s) URL" — it forced a hard page reload instead of client routing, and showed a misleading ↗ icon on a route that never left the site. Fixed: `ProtoLink` now branches three ways (genuine `http` external → arrow + native new-tab; internal route with an `href` → client-side `navigate()`, no arrow; no `href` → case-study morph).
- **NIT.** `dialkit`/`agentation`/`motion` were in `dependencies` instead of `devDependencies` — correct per the project's own "Dev panels policy" (CLAUDE.md) even mid-iteration, since these are explicitly dev-only and stripped before merge. Moved.
- **Deferred** (captured in `plan.md`, not fixed here — see FOLLOW-UP/roadmap-concrete/future-exploration items there): wiring the real `useGlassHighlight` glass-pill system into the panel's control rows (push-further, roadmap-concrete — bigger assembly task); collapsing accent+intensity into one gesture (push-further, future-exploration); `PreviewContext` state-machine tests; settings-panel overflow on short viewports; a keyboard-timing edge case in the morph-vs-hard-cut decision; 24×24 swatch touch targets (pre-existing, matches the real sidebar, not a new regression).

**Verification:** `tsc -b`, `vite build`, and 72 tests all clean. Branch rebased onto `origin/main` (was 2 commits behind — automated `contributions.json` updates only) before running `/flow:staff-review`, per its stale-base gate. All three BLOCKER fixes verified with live CDP interaction (not just code-reading) after several failed attempts earlier in the session — the working recipe: absolute local paths (not `cd &&` chains, which silently no-op after any earlier command times out in this sandbox), per-`send()` timeouts so a stuck `Runtime.evaluate` can't hang the whole script, and a genuinely patient wait (10-12s) before the first interaction on a cold load of this dependency-heavy page.

---

## 2026-08-19 — V2 home prototype round 3: DialKit tuning dials, anchored settings panel, cursor row, Agentation

**Branch:** `simplified-home-prototype`. Continues the `/new` prototype (see round 1/2 entries below).

**Summary:** Installed [DialKit](https://joshpuckett.me/dialkit) for live-tunable values and reworked the customization panel to be anchor-stable per Ben's feedback, plus a batch of smaller fixes from a screenshot review.

- **DialKit tuning dials** (`src/proto/tuning.ts`, dev-only — strip before merge) — three live panels (Type, Page transitions, Hover preview) covering: heading/body text size + heading line-height (the thumbnail derives its square size from `headingSize × headingLineHeight`, per Ben's ask); hover-preview size (area-based, aspect-ratio-preserving) + rise + scale-from + enter easing + morph spring + fade-out; page enter/exit duration + rise + hero-shrink-on-exit. `<DialRoot />` renders the floating panel in `ProtoRoutes.tsx`.
  - **Gotcha (real, not flaky):** `useDialKit`'s config argument must be a **stable reference** — passing a fresh object literal every render (as DialKit's own README examples do) defeats its change-detection and caused a genuine infinite render loop (confirmed via `--dump-dom` hanging indefinitely, vs. resolving instantly after the fix). Fixed with a `useRef`-backed `useStableConfig` helper; tuple/literal fields need `satisfies DialConfig` to avoid TS widening `[30, 20, 44, 1]` to `number[]`.
  - **Gotcha:** DialKit installs its own nested `motion`→`framer-motion@13`, distinct from the app's top-level `framer-motion@11` (`npm ls motion` shows both) — DialKit's `TransitionConfig` (`type: 'easing'`) is structurally but not nominally compatible with the app's `AnimationOptions` (`type: 'tween'`). Adapted at one boundary function (`toMotionTransition`) rather than fighting cross-package type identity; `animate()` narrowed to a single-value-overload alias in `PreviewLayer.tsx` so call sites stay typed instead of falling to `any`.
- **Settings-panel anchor fix** (Ben: "clicking the thumbnail shouldn't make it change position; use the position as an anchor for a smooth transition; it's not obvious you need to click it again to go back"). The thumbnail `<button>` is now a genuinely fixed anchor — same DOM element, same `position: absolute; top: 2; left: 0`, never moves — while its **content** swaps (portrait dims + a Phosphor `X` icon fades in) and the prose block fades to `opacity: 0` (kept mounted, not unmounted, so the column height and everything's position stays stable) while the settings panel fades in below. Escape key also closes it.
- **Accent-reactive thumbnail** (Ben: "other images should be shown on theme change, which correspond to colors") — thumbnail now sources `/images/portrait-${accentColor}.jpeg`, swapping with the rest of the environmental shift instead of always showing the table portrait.
- **Cursor row added** (Ben: "figpal cursor customization is missing") — Standard/Figpal buttons wired to the real `CursorContext`; `<CustomCursor />` now renders inside `ProtoRoutes` so the mode picker actually does something. Omitted the Invert mode (no natural fit without the rest of the invert-cursor system) and kept the row to two, matching what's meaningfully togglable here.
- **Swatch colors read from `theme.css`** (`var(--swatch-${accent})` / `var(--swatch)`) instead of a hardcoded hex table — closes a drift hazard a reuse review flagged (a future palette tweak in `theme.css` would otherwise silently desync the prototype).
- **Agentation installed** (`agentation` npm package, Ben: "install agentation in this preview as well") — `<Agentation />` mounted alongside `<DialRoot />` in `ProtoRoutes.tsx` for click-to-annotate visual feedback during review.
- **Arrow rendering fix:** tried `@phosphor-icons/react`'s `ArrowUpRight` inline in the prose links (a reuse-review suggestion, replacing a hand-rolled inline SVG) but it introduced a real layout bug — the icon triggered a spurious line-break before itself in narrow paragraph flow (Chrome will break before an atomic inline-replaced box even inside a `white-space: nowrap` ancestor, in a way six-line hand-rolled inline SVGs with `display: inline-block` don't). Reverted to the hand-rolled SVG for the *inline prose* arrow specifically; kept Phosphor for the non-inline settings-panel icons (Monitor/Sun/Moon/NavigationArrow) where this failure mode doesn't apply.
- **Simplify pass** (`/simplify`, 4 parallel review agents — reuse/simplification/efficiency/altitude) applied: dropped the now-unused `return`/`docked` phase split (state machine is `hidden → hover → morph → hidden`), dropped the placement/entrance dev-knob system entirely per Ben's request (replaced by one fixed "mostly appear, slight rise + scale from the link" entrance — no more fly-in-from-elsewhere, ever), CSS-only underline hover (`proto.css`, `[data-proto-link]:hover span`) replacing a `querySelector`-based JS handler, mini-preview media kept mounted-but-hidden across hover cycles (fixes video restart + re-fetches the Lottie JSON once via a module-level cache instead of per-hover), single actions-shaped context split isn't done (judged not worth it at prototype scale) but `registerLink` no longer bumps a re-render-triggering version counter.
- **Dev-server gotcha (new class, not covered by existing feedback.md entries):** after adding new dependencies mid-session (dialkit, agentation, a new `@phosphor-icons/react` import), the running Vite dev server can serve a **completely empty `<div id="root">`** — no error overlay, no console output, `vite build` and `tsc` both clean — because its dependency pre-bundle cache is stale and the re-optimization reload races the initial page load. Fixed with `rm -rf node_modules/.vite && vite --force`. Confirmed via `google-chrome --headless --dump-dom` (reliable in this sandbox) that the root div was empty before the fix and populated immediately after — this is a **dev-server cache-staleness gotcha**, distinct from the HMR-blank-page issue already in `feedback.md` (2026-03-28), which is about *edits during* a running session; this one is about *new dependencies* added mid-session.

**Verification:** `tsc -b` clean for all `src/proto/*` files (pre-existing unrelated errors in `ui-playground/`/`PlaygroundDemo.tsx` untouched); `vite build` clean; 72/72 tests pass. Confirmed via `--dump-dom` (root div populated, correct `text-indent: 46px` on the h1 matching `36×36` thumbnail + `10px` gap) and screenshot (home page, DialKit panel with live values, inline arrows correctly positioned) on a freshly-optimized dev server. Case studies (`/new/cip`, `/new/sony`) confirmed rendering post-refactor. Settings-panel anchor stability and click-to-toggle affordance verified structurally (single persistent `<button>` element, position never in its own style) rather than via a live click — CDP-based click automation was unreliable in this sandbox session (repeated hangs across many script variants); this is a **known verification gap**, worth a manual click-through before merge.

**Open for Ben:** DialKit-tuned values above are current defaults, not final — turn the dials in-browser and report back what feels right; those values then get baked into the components and DialKit/Agentation get stripped per the dev-panels policy in `technical-context.md`.
