...
import {
  ...
  SLOT_HOVER_GAP,
  COIN_DIAMETER,
} from "./arcade-internal/coinPhysics";
import { arcadeAudio } from "./arcade-internal/audio";
import "./arcade-internal/arcade-themes.css";
...
function siblingHue(accentHue: number, index: number): number {
  const offset = DEMO_OFFSETS[index % DEMO_OFFSETS.length] ?? 0;
        ...
        >
          {galleryDemos.map((demo, i) => (
            <CabinetTile
              ...
              demo={demo}
              index={i}
              cabinetHue={cabinetHues[i] ?? demo.hue}
              state={states[demo.path] ?? "idle"}
